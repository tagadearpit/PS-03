...
export default function AuthPage({ mode, onSuccess }: { mode: AuthMode; onSuccess: (user: AuthUser) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    window.setTimeout(() => {
      ...
      if (isLogin && saved && saved.email !== normalizedEmail) { toast.error("No local account found for this email.", { description: "Create an account first to continue." }); setBusy(false); return; }
      const user = saved && saved.email === normalizedEmail ? saved : { email: normalizedEmail, name: name.trim() || normalizedEmail.split("@")[0] };
      markSignedIn(user);
